...
3. retain the full source/authority, goal, outcome, coverage, truth, deliverable, risk/cost, structured
   ROOT-to-worker role-authority graph,
...
This artifact split does not relax the strict orchestrator-to-worker boundary. ROOT remains the sole
decision owner and concretely authors every governing card and every member worker card before
dispatch. Workers may choose implementation or investigation mechanics only inside that complete
contract. They never define task meaning, scope, required behavior, success, acceptance, correction
classification, routing, integration, real attempts, or follow-on dispatch, and they never direct
other workers. Every completed worker returns a terminal handoff to ROOT before ROOT makes the next
acceptance, correction, integration, or graph decision.
