            ...
            for task_field in TASK_FIELDS:
                ...
                if invalid_executable_value(raw_task_value):
                    errors.append(
                        ...
                        "ROOT must supply a concrete dispatch-contract value; N/A and "
                        "justification-based waivers are forbidden"
                    )
...
def validate_member_lane_and_handoff_manifest(
    ...
    role_parent = {
        ...
        if len(row) == len(ROLE_TABLE) and normalize(row[1]).upper() == "WORKER"
    }
    ...
    for card_id, instance_id, role, lane_id, handoff_id, process_id, invocation_id in member_contracts:
        ...
        if lane_row is None:
            ...
            errors.append(
                f"{instance_id} {card_id} lane {lane_id} Consumer must be its ROOT owner "
        ...
        if handoff_row is None:
            ...
            errors.append(
                f"{instance_id} {card_id} handoff {handoff_id} Consumer must be its ROOT owner "
                f"{role_parent[role]!r}"
            )
...
def validate_card_authority_and_graph_references(
    ...
    for card_kind, card_id, card_body in task_card_blocks(joined_instances):
        ...
        if card_kind == "Governing" and authority_class != "ROOT":
            errors.append(f"governing task card {card_id} must use the ROOT role")
...
def validate_step_files(
    ...
    for row in step_rows:
        ...
        if not root_role or row[5] != root_role:
            errors.append(f"{step_id} Acceptance owner must be the ROOT role {root_role!r}")
    ...
    for step_id, indexed_path in indexed_steps.items():
        ...
        if not root_role or contract_values.get("Acceptance owner") != root_role:
            errors.append(f"{filename} Acceptance owner must be the ROOT role {root_role!r}")
