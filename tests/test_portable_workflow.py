...
# shim) instead of Git for Windows' bash. That shim cannot accept a plain
# Windows-drive path, so prefer Git Bash explicitly for the Windows-only hook
# tests. Native Unix-like tests always use the host's Bash—never WSL.
