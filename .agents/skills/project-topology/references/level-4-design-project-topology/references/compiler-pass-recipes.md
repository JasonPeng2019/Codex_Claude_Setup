...
4. Define each role's authority class, reciprocal `Reports to`/`Directs` edges, responsibilities,
   ...
   condition, and structured-handoff fallback. Declare exactly one ROOT; every other role is a direct
   WORKER child and directs no role.
...
- no worker role owns missing task semantics, acceptance, correction classification, or self-dispatch;
  review/audit discovery is open only inside its concrete assigned boundary; and
- the structured authority graph has exactly one ROOT, only direct WORKER children, reciprocal edges,
  and no worker with children.
...
- every internal worker has exactly one complete ROOT-authored member card, and every review/audit member card is a
