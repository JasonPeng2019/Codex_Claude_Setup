...
product. Sampling requires a reason the omitted interactions do not hide a required
failure; pairwise testing is not automatically sufficient. Preserve required security,
...
2. Dispatch a separate read-only reviewer, independent of the draft writer. Give it all
   ...
   which evidence is missing, stronger than required, duplicated, or unnecessarily
   expensive? It may recommend retaining everything. Never require a removal quota.
...
4. The writer validates every criticism. Record ACCEPT-REMOVE, ACCEPT-MERGE,
...
6. ROOT accepts the final scope only after every finding has a disposition and required
   coverage is established. If review is unavailable or a material requirement remains
