# Portable multi-agent workflow for Codex, Claude Code, and CLI workers
...
lifecycle hooks, proportional verification, optional delegation/review, safe
worktree/commit workflows, and an opt-in provider-neutral CLI-worker launcher.
Codex and Claude Code receive the same engineering policy and the same hook
behavior, driven by one shared set of scripts under `.agent/`; only the thin
...
- `.agent/multi-agent.json` — a reviewed, argument-vector catalog of CLI worker
  ...
  executable and arguments are replaced for the local provider; `claude-cli` and
  `codex-cli` ship disabled too, ready to enable once reviewed.
- `.agent/roles/` — small task-card guidance for investigator, implementer,
  reviewer, and verifier roles. Roles define write authority without naming a
  provider or model.
- `.agent/multi_agent.py`, `.agent/launch-agent.ps1`, and
  `.agent/launch-agent.sh` — provider-neutral worker preparation, foreground or
  background launch, process supervision, heartbeat output, and durable status
  and result records under `.agent-runtime/multi-agent/`.
- `.agent/pipe-prompt-file.sh` and `.agent/pipe-prompt-file.ps1` — a small
  bridge for a worker CLI (such as `claude -p` or `codex exec`) that reads its
...
Direct `codex exec` and `claude -p` workers, native subagents, and this
repository's agent-launch wrappers remain unbounded. Do not place them inside
`run-bounded` or list them in `.agent/bounded-commands.txt`. Add only a
high-confidence finite test, script, build, server, watcher, or other command
fragment to that file.
...
already uses that harness; it is never required for normal work. Direct
ROOT-to-worker dispatch remains the default. When its coordination payoff is
explicit, ROOT may instead authorize one direct lane-sub-orchestrator tier whose
workers are terminal; a third orchestration tier is forbidden.
...
### Provider-neutral CLI workers

The optional launcher does not know or select a provider. It accepts a reviewed
array of command arguments, expands a small set of task-card placeholders, and
starts that local executable without a shell. Configure a CLI by replacing and
enabling `generic-cli` in `.agent/multi-agent.json`, for example:

```json
"my-local-agent": {
  "enabled": true,
  "command": ["my-agent", "run", "--prompt-file", "{prompt_file}"],
  "roles": ["investigator", "reviewer", "verifier"],
  "environment": {}
}
```

The permitted placeholders are `{prompt_file}`, `{role}`, `{task_id}`,
`{workspace}`, and `{delegation_dir}`. Every entry must include
`{prompt_file}`. This makes prompt delivery explicit and lets any CLI participate
as long as it can be invoked with a fixed argument vector.

#### Wiring `claude -p` or `codex exec`

Neither `claude -p` nor `codex exec` accepts a file path for its prompt; both
read the prompt from stdin when no prompt argument is given. Since the
launcher only ever substitutes `{prompt_file}` as a literal argument, the
shipped `claude-cli` and `codex-cli` entries in `.agent/multi-agent.json`
route through a small bridge, `.agent/pipe-prompt-file.sh` (a `.ps1` twin
exists for a pure-PowerShell environment without `bash`), which reads the
file and re-execs the real worker with its bytes on stdin:

```json
"claude-cli": {
  "enabled": false,
  "command": [
    "bash", "{workspace}/.agent/pipe-prompt-file.sh", "{prompt_file}",
    "--", "claude", "-p"
  ],
  "roles": ["investigator", "implementer", "reviewer", "verifier"],
  "environment": {}
}
```

Both entries ship **disabled**, exactly like `generic-cli`: a repository owner
must review and enable one before the launcher will run it. Non-interactive
`-p`/`exec` runs have no terminal to answer a permission prompt, so an enabled
entry that needs tool access beyond what the host's default permission model
already allows will hang or fail until the repository owner configures that
access themselves (for example `claude`'s `--dangerously-skip-permissions` or
`--allowedTools`, or Codex's own sandbox/approval flags) — this template does
not add such a flag on your behalf, since accepting that risk is a
repository-owner decision, not a default.

Validate the catalog and role templates before launch:

```powershell
python .agent/multi_agent.py validate
```

On macOS or Linux, use `python3 .agent/multi_agent.py validate` when `python`
is not a Python 3.10+ command. The Bash launcher likewise tries `python3`, then
`python`, or uses an explicit `PYTHON` executable.

Use a read-only investigator in the current repository:

```powershell
& .agent/launch-agent.ps1 -Agent my-local-agent -Role investigator `
  -Task 'Find the source of the flaky parser test' `
  -Scope 'tests/parser and src/parser only' `
  -AcceptanceCheck 'Return evidence-backed findings; do not edit files'
```

On macOS or Linux, invoke the same worker through the Bash entrypoint:

```bash
bash .agent/launch-agent.sh --agent my-local-agent --role investigator \
  --task 'Find the source of the flaky parser test' \
  --scope 'tests/parser and src/parser only' \
  --acceptance-check 'Return evidence-backed findings; do not edit files'
```

Use a writer only from a worktree. The launcher rejects a writer whose working
directory is the main workspace unless `-AllowSharedWorkspace` or
`--allow-shared-workspace` is consciously supplied:

```powershell
& .agent/launch-agent.ps1 -Agent my-local-agent -Role implementer `
  -Task 'Implement the accepted parser fix' `
  -WorkingDirectory 'C:\worktrees\parser-fix' `
  -Scope 'src/parser/**' `
  -AcceptanceCheck 'Run the parser-focused tests'
```

The corresponding macOS/Linux form uses a worktree path:

```bash
bash .agent/launch-agent.sh --agent my-local-agent --role implementer \
  --task 'Implement the accepted parser fix' \
  --working-directory ../parser-fix \
  --scope 'src/parser/**' \
  --acceptance-check 'Run the parser-focused tests'
```

Add `-Background` to return after the unbounded supervisor starts. It emits
heartbeats and writes `delegation.json`, `prompt.md`, worker logs, `status.json`,
and `result.json` to a unique runtime directory until the worker exits. Inspect a
worker with `python .agent/multi_agent.py status --id <delegation-id>`. A task
...
- Review and enable only local CLI command entries you intend to launch in
  `.agent/multi-agent.json`; keep their role list narrow and use a worktree for
  implementers.
