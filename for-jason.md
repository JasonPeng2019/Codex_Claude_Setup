...
This copy adds a small, provider-neutral way to delegate a well-scoped task to
any local agent CLI. It does not choose a provider or model. You review one
command-array entry in `.agent/multi-agent.json`, enable it, choose a role such
as investigator, implementer, reviewer, or verifier, and launch it through
`.agent/launch-agent.ps1` or `.agent/launch-agent.sh`.

Each worker gets a concrete task card and leaves a durable record in
`.agent-runtime/multi-agent/`: its scope, prompt, command result, and
stdout/stderr logs. The launcher starts the CLI without a shell, emits
heartbeats, and remains unbounded until the worker exits. Read-only roles can use
the main checkout. Writers need a separate worktree unless the caller consciously
overrides that guard.

The primary agent still owns decisions, integration, and final verification.
This is lean delegation support, not an always-on orchestration service.
...
does not become the default process for every project. Direct ROOT-to-worker
dispatch remains the default; one optional direct lane-sub-orchestrator tier is
allowed when justified, and its workers are terminal.
...
force every quick command—or any `codex exec`, `claude -p`, native subagent, or
repository agent-launch wrapper—through it.
